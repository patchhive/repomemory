      ...
      <div style={{ ...S.panel, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ color: "var(--text-dim)", fontSize: 12 }}>
            RepoMemory is a local memory layer first. These checks focus on GitHub readiness, auth posture, and storage health.
          </div>
        </div>
      ...
      {health && (
        <div style={{ ...S.panel, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 700 }}>{health.github_ready ? "yes" : "no"}</div>
          </div>
          <div>
            <div style={S.label}>Tracked repos</div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>{health.counts?.repos ?? 0}</div>
          </div>
          <div>
            <div style={S.label}>Runs</div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>{health.counts?.runs ?? 0}</div>
          </div>
          <div>
            <div style={S.label}>Memories</div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>{health.counts?.memories ?? 0}</div>
          </div>
